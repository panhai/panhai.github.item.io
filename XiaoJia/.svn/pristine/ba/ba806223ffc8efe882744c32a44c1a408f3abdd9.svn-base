...
function scrollNav(navheight){
	var windowHeigth = $(window).height(); //屏幕高度
	var navHeights = $('.nav').height() + $('.school-infor').height() + $('.logoBox').height()  //左侧导航的高度
// 	var bol = true;
...
// if(navHeights >= windowHeigth-30){
//     console.log($(window).height(),$('.left-navgater').height())
//     $('.nav').css({'height':navheight+20,'padding-bottom':'30px'})
...
$(function(){
    // var navheight = $('.left-navgater').height();
    // // scrollNav(navheight)
    
    $(".left-navgater").mCustomScrollbar();
...
//弹框按钮选中样式
$('.plus-and-select .table-piece table tr td:last-child a').click(function(){
    if($(this).hasClass('change')){
        $(this).removeClass('change').text('选择');
    }else{
        $(this).addClass('change').not(this).removeClass('change').text('已选');
