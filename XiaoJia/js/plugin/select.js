...
//岗位选项初始化
window.initJobSelect = function (jobDom) {
    if (jobDom.length == 0) return false;

    //选择部门
    jobDom.find('.deptlist span').click(function () {
    ...
    //返回部门选择
    jobDom.find('.head .dept').click(function () {
...
window.initJobSelect($('.DEPARTMENT'));

/*加上SelectAll后，初始化时默认不选中“全部”*/
window.initMultiSEL = function (MultiSELDom) {
    if (MultiSELDom.length == 0) return false;
    ...
    function Init(selectors) {
        ...
        selectors.each(function () {
            ...
            if (ids == '' || ids.length == 0) {
                ...
                if (!selectors.hasClass("SelectAll")) {
    ...
    $('.Multiselect .multilist .unlimited', MultiSELDom).unbind("click").click(function () {
        $(this).addClass('cur');
        $(this).parent().siblings('.state').children().removeClass('cur');
    });

    $('.Multiselect', MultiSELDom).off("click", ".state a").on("click", ".state a", function () {
    ...
    $('.Multiselect .option_btn .c_confirm', MultiSELDom).unbind("click").click(function () {
        var SELECT = $(this).closest('.SELECT');
        var alist = SELECT.find('a.cur');
        ...
        if (idlist != '') {
            ...
            if (SELECT.hasClass("SelectAll") && $('a.unlimited', SELECT).hasClass('cur')) {
                SELECT.children('dt').html($.trim($('a.unlimited', SELECT).text()));
            } else if (dtPlaceholder) {
    ...
    $('.Multiselect .option_btn .c_cancel', MultiSELDom).unbind("click").click(function () {
        $(this).closest('.SELECT').removeClass('SELECT-on');
    });
...
var SELECT = {
    ...
    reset: function (bChange, bSaveValue) {
        ...
        else $("dd a", this).removeClass("cur");
    ...
    value: function () {
        ...
        sTexts = sTexts.join(',');
        oJQchoicesList.find("a").not(oItems).removeClass("cur");
            ...
            if ($(this).data('anchor') != 'save') {
                ...
                oJQchoicesList.delegate('a', 'click', function () {
                    ...
                    if (piChoices == 1) {//单选，将自己设置为选中状态，其它所有选项取消选中
                        ...
                        $(this).addClass('cur');
                        var par;
                        (par = $(this).closest("tbody")).length && par.closest(".SELECT").length && (par.find("a").not(this).removeClass("cur"), true)
                        ...
                        if (oCollectedItem.size()) {//已经存在，则删除
                            ...
                            oJQselect.find("a[data-id=" + piID + "]").removeClass("cur");
                            //$(this).removeClass("cur");
                        }
                        else {//不存在
                            ...
                            if (oItems.size() >= piChoices && piChoices) {//数量满，则提示
                                ...
                                oJQcollector.find("p:first").before('<a href="javascript:void(0)" data-id="' + piID + '" class="cur">' + sText + '</a>');
                                $(this).addClass("cur");
                            }
                        }
                    }
                ...
                if (piChoices != 1) {
                    oJQcollector.delegate('a q', 'click', function () {
                        var oJQa = $(this).parent(),
                            piID = oJQa.data("id");
                        oJQa.remove();
                        oJQchoicesList.find('a[data-id="' + piID + '"]').removeClass("cur");
                    })
                        //点击确定
